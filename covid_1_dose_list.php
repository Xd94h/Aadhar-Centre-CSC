...
if(isset($_GET['track']) && $_GET['track'] =="true" && $_GET['application_no']){
    ...
    // print_r($_GET);
            ...
            $url = 'https://apizone.in/api/v1/services/covid_1_dose/track.php?application_no='.$application_no.'&api_key='.$api_key;
            $curl = curl_init();
            ...
            if($resdata['status'] == 1){
                $sql = mysqli_query($ahk_conn,"UPDATE covid_1_dose SET pdf_link='".$resdata['pdf_link']."', status='success' WHERE application_no='".base64_decode($application_no)."'");
                if($sql){
                    ?>
                <script>
                    $(function(){
                    ...
                    setTimeout(() => {
                        window.location='covid_1_dose_list.php';
                    }, 1200);
                </script>
                    <?php
                }else{
...
$res = mysqli_query($ahk_conn,"SELECT * FROM covid_1_dose WHERE appliedby='".$udata['phone']."'  ORDER BY id DESC");
