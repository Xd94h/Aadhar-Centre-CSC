...
include('../template/ahkweb/rasan_pdf_admin_list.php');
if(isset($_POST['aadhaar_no']) && !empty($_POST['aadhaar_no']) ){
 ...
 $id = mysqli_real_escape_string($ahk_conn,$_POST['id']);
   ...
   if(move_uploaded_file($_FILES['aadhaar_pdf']['tmp_name'],$upload.$ext)){
    $sql = mysqli_query($ahk_conn,"UPDATE rasan_card SET pdf_link='$link', aadhaar_no='$aadhaar_no',status='success' WHERE id='$id'");
    if($sql){
        ?>
        <script>
            $(function(){
