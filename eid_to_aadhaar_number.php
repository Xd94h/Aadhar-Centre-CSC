...
include('../template/ahkweb/eid_to_aadhaar_number.php');
if(isset($_POST['eid']) && !empty($_POST['name']) && !empty($_POST['date']) && !empty($_POST['time']) && !empty($_POST['ret_wp_no']) ){
    ...
    $price = mysqli_fetch_assoc(mysqli_query($ahk_conn,"SELECT * FROM pricing WHERE service_name='aadhaar_number' "));
    if($udata['balance']>=$price['price']){
        ...
        $submit = mysqli_query($ahk_conn,"INSERT INTO `aadhaar_number` (`appliedby`, `name`, `eid`, `date`, `time`, `status`, `ret_wp_no`,`fee`,`application_no`) VALUES('$appliedby','$name','$eid','$date','$time','pending','$ret_wp_no','$fee','$application_no')");
        if($submit == true){
           ?>
                <script>
                    $(function(){
